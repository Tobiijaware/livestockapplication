...
namespace AnimalFarmsMarket.Data.Services.Implementations
{
    public class ReviewService : IReviewService
    {
        ...
        public async Task<AppUser> GetReviewerOnALivestock(string livestockId)
        {
            var res = await _ctx.Reviews.Include(s => s.User).Where(x => x.LivestockId == livestockId).Select(s => s.User).SingleOrDefaultAsync();
            return res;
        }
